/*************************************************************************
	cs3723p1.c by Jonathan Jimenez
Purpose:
	Collection of functions invoked to either allocate memory, add/remove
	reference, associate a node to another, and print all attributes of the nodes.	
...
void smrcRemoveRef(StorageManager *pMgr , void *pUserData, SMResult *psmResult)
{
	...
	if (pNewNode->shRefCount == 0)
	{
		...
		//Using the for loop condition from setData function to loop through the attributes of the node.
		...
		{			
			...
			if(pMetaAttr->cDataType == 'P')
			{
				...
				if (!pAttr)
				{
					...
					strcpy(psmResult->szErrorMessage, "Address is not in the heap ");
				}
				//Recursively calls this function if the pointer is not pointing to NULL
				if (*pAttr != NULL)
					smrcRemoveRef(pMgr, *pAttr, psmResult); 
				smFree(pMgr, pNewNode, psmResult);
				//invoke smFree function to free the node and set reference coutn to -1
			}
		}
...
void smrcAssoc(StorageManager *pMgr , void *pUserDataFrom, char szAttrName[], void *pUserDataTo, SMResult *psmResult)
{
	...
	//Using for loop condition from the setData function to loop through the attributes of the node.
...
void printNode(StorageManager *pMgr, void *pUserData)
{
	...
	//Using setData function for loop condition to loop through all attributes of the node.
	for (int iAt = pMgr->nodeTypeM[pNewNode->shNodeType].shBeginMetaAttr; pMgr->metaAttrM[iAt].shNodeType == pNewNode->shNodeType; iAt++)
